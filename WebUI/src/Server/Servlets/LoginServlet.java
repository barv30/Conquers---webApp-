...
import static Server.Constants.Constants.LOGIN_ERROR_URL;

@WebServlet(name = "LoginServlet", urlPatterns = {"/login","/pages/LoginError/login"})
public class LoginServlet extends HttpServlet {
   ...
   // must start with '/' since will be used in request dispatcher...
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        {
            signedUpUser(request,response);
        }
        else{
            loginWasPressed(request,response);
        }
    }

    private void loginWasPressed(HttpServletRequest request, HttpServletResponse response) throws IOException, ServletException {
        String usernameFromSession = SessionUtils.getUsername(request);
        UserManager userManager = ServletUtils.getUserManager(getServletContext());
        if (usernameFromSession == null) {
            //user is not logged in yet
            String usernameFromParameter = request.getParameter(USERNAME);
            if (usernameFromParameter == null|| usernameFromParameter.isEmpty()==true) {
                ...
                synchronized (this) {
                    if (userManager.isUserExists(usernameFromParameter)) {
                        ...
                        response.sendRedirect(request.getContextPath()+ LOBBY_URL);
                    }
                }
            }
        } else {
    ...
    private void signedUpUser(HttpServletRequest request, HttpServletResponse response) throws IOException {
        Gson gson = new Gson();
        PrintWriter out = response.getWriter();
        String redirect =  request.getContextPath()+LOBBY_URL;
        response.setContentType("application/json");

        if(/*SessionUtils.hasSession(request)*/SessionUtils.getUsername(request)!=null) {
